...
def constant_speed_trajectory(velocity: np.ndarray, initial_state: dict):
    ...
    for t in np.arange(dt, 3, dt):
        ...
        joint_velocities.T
        track["time"].append(t)
        track["theta_0"].append(theta_0 + float(joint_velocities[0]) * dt)
        track["theta_1"].append(theta_1 + float(joint_velocities[1]) * dt)
        track["theta_2"].append(theta_2 + float(joint_velocities[2]) * dt)
        track["theta_3"].append(theta_3 + float(joint_velocities[3]) * dt)
